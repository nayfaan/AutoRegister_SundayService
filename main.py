...
def fillForm(driver,members):
    ...
    try:
        for member in members:
            fellowshipTemp = tags['fellowship'] + 'label.radio>input[value="'+ str(member['fellowship']) +'"]'
...
def fellowshipConversion(memberDict):
    ...
    if not memberDict['fellowship'] in fellowshipList:
        raise Exception(memberDict['firstName'] + '' + memberDict['lastName'] + '\'s fellowship: "' + memberDict['fellowship'] +'" is not valid.')
...
def readFile():
    ...
    with open(INPUT_PATH, 'r', newline='') as csvfile:
        ...
        members_csv = csv.DictReader(csvfile, delimiter=',', quotechar='"', quoting=csv.QUOTE_ALL)
        ...
        for member in members_csv:
            if member:                
                ...
                if not member['lastName']:
                    raise Exception('Last name is missing for ' + member['firstName'] + '.')
                if not member['phone']:
                    raise Exception('Phone no. is missing for ' + member['firstName'] + ' ' + member['lastName'] + '.')
                if not member['email']:
                    raise Exception('E-mail is missing for ' + member['firstName'] + ' ' + member['lastName'] + '.')
...
def run():
    ...
    initiate_driver(driver, "https://nycbc.breezechms.com/form/6d9d2510867913525845518213115168941257227345259327618061713965257424945628127140573113171039894790")
